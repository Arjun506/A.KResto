...
import AuthLayout from '@/components/auth/AuthBackground'; // Let's use our AuthLayout
import SharedAuthLayout from '@/components/auth/AuthLayout';
...
function VerifyEmailContent() {
  ...
  const handleVerify = async () => {
    setLoading(true);
    setErrorMsg(null);
    const token = searchParams.get('token') || 'verify_demo_user';
    ...
    try {
      ...
      console.warn('API verification offline. Simulating validation...', err);
      setTimeout(() => {
        setSuccess(true);
        setTimeout(() => {
          router.push('/login');
    ...
    <div className="space-y-6 text-center">
      ...
      {!success ? (
        <div className="space-y-6">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-955 dark:text-white">Verify Your Identity</h2>
          ...
          <div className="text-xs font-bold text-slate-655 dark:text-slate-450 border-t border-slate-200/30 dark:border-white/5 pt-5 flex items-center justify-between">
    ...
    <SharedAuthLayout>
      <Suspense fallback={
        <div className="text-center py-12 flex flex-col items-center justify-center">
          <Loader2 className="animate-spin text-blue-600 dark:text-cyan-400 mb-4" size={24} />
