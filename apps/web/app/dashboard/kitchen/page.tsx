...
const stationForItem = (name: string) => {
  const normalized = name.toLowerCase();
  if (normalized.includes('pizza') || normalized.includes('pasta')) return 'Pizza Station';
  if (normalized.includes('drink') || normalized.includes('coffee') || normalized.includes('mojito')) return 'Beverage Station';
  if (normalized.includes('salad')) return 'Salad Station';
  if (normalized.includes('dessert') || normalized.includes('brownie')) return 'Dessert Station';
  if (normalized.includes('fried') || normalized.includes('fries') || normalized.includes('roll')) return 'Fry Station';
  return 'Grill Station';
};

const toKitchenStatus = (status: string): LocalOrder['status'] => {
...
const mapApiOrderToLocal = (order: Order): LocalOrder => {
  ...
  return {
    ...
    source: 'QR',
    priority:
      Date.now() - new Date(order.createdAt).getTime() > 12 * 60 * 1000
        ? 'High'
        : 'Normal',
    ...
    items: order.items.map((item) => {
      ...
      return {
        ...
        station: stationForItem(name),
      };
    }),
    status: toKitchenStatus(order.status),
    createdAt: order.createdAt,
    estimatedCompletionMinutes: 15,
    qualityChecked: order.status === 'READY' || order.status === 'COMPLETED',
    packed: order.status === 'READY' || order.status === 'COMPLETED',
  };
};

const upsertLocalOrder = (
  current: LocalOrder[],
  order: Order,
): LocalOrder[] => {
  const mapped = mapApiOrderToLocal(order);
  const existing = current.find((item) => item.id === mapped.id);
  const merged = existing
    ? {
        ...mapped,
        chefAssigned: existing.chefAssigned,
        stationAssigned: existing.stationAssigned,
        qualityChecked: existing.qualityChecked || mapped.qualityChecked,
        packed: existing.packed || mapped.packed,
        cancellationReason: existing.cancellationReason,
      }
    : mapped;

  const next = current.filter((item) => item.id !== mapped.id);
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      ...
      try {
        ...
        if (active) {
          setOrders(data.map(mapApiOrderToLocal));
        }
      } catch (err: any) {
        if (active) {
          setOrderError(err?.message || 'Failed to load kitchen orders.');
        }
      } finally {
        if (active) {
    ...
    const handleOrder = (payload: Order) => {
      if (payload?.id) {
        setOrders((current) => upsertLocalOrder(current, payload));
      }
    };
    ...
    socket.on('orderCreated', handleOrder);
    socket.on('orderUpdated', handleOrder);
    socket.on('orderStatusChanged', handleOrder);
    socket.on('kitchenOrderSync', handleOrder);
    socket.on('orderDeleted', handleDeleted);

    return () => {
      active = false;
      socket.off('orderCreated', handleOrder);
      socket.off('orderUpdated', handleOrder);
      socket.off('orderStatusChanged', handleOrder);
      socket.off('kitchenOrderSync', handleOrder);
