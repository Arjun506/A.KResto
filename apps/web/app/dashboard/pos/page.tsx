...
import { createOrder, checkoutOrder, getOrders } from '@/services/order.service';
import type { Order as ApiOrder } from '@/src/types/order.types';
...
import {
  ...
  addRegisterCashLog,
} from '@/services/pos-register.service';
...
const toTableStatus = (status: string | undefined): TablePOS['status'] => {
...
const toLiveOrderStatus = (status: string): LiveOrder['status'] => {
...
const mapApiOrderToLiveOrder = (
  ...
  return {
    ...
    tableName: tableNameById.get(order.tableId) || 'Table',
    ...
    serviceCharge: 0,
    total: subtotal,
    status: toLiveOrderStatus(order.status),
...
export default function POSPage() {
  ...
  const reloadRegisterSession = async () => {
    try {
      ...
      if (session) {
        ...
        const backendLogs = (session.cashLogs || []).map((l: any) => ({
          id: l.id,
          type: l.type === 'CASH_IN' ? 'IN' : 'OUT',
          amount: Number(l.amount),
          reason: l.reason,
          time: new Date(l.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        }));
        
        const backendCashPayments = (session.payments || [])
          .filter((p: any) => p.paymentMethod === 'CASH')
          .map((p: any) => ({
            id: p.id,
            type: 'IN',
            amount: Number(p.amount),
            reason: `Order payment - Cash`,
            time: new Date(p.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          }));

        const allTxs = [...backendLogs, ...backendCashPayments];
        setCashTransactions(allTxs);
      } else {
        setShiftOpened(false);
      }
    } catch (err) {
      console.warn('Failed to load active POS session:', err);
    }
  };

  useEffect(() => {
    void reloadRegisterSession();
  }, []);

  const handleOpenSession = async () => {
    setRegisterBusy(true);
    try {
      await openRegisterSession(Number(openingBalanceInput));
      triggerToast('Cash register opened successfully.');
      void reloadRegisterSession();
    } catch (err) {
      console.error(err);
      triggerToast('Failed to open cash register.');
    } finally {
  ...
  const handleCloseSessionSubmit = async () => {
    ...
    try {
      await closeRegisterSession({
        closingBalance: Number(closingBalanceInput),
        notes: closingNotesInput,
      });
      triggerToast('Cash register session closed.');
      ...
      console.error(err);
      triggerToast('Failed to close cash register session.');
    } finally {
      setRegisterBusy(false);
    }
  };
  ...
  const loadMenuData = async () => {
    try {
      setLoading(true);
      const [items, cats, tablesData, ordersData] = await Promise.all([
        getMenuItems().catch((): MenuItem[] => []),
        getCategories().catch((): MenuCategory[] => []),
        getTables().catch((): any[] => []),
        getOrders().catch((): ApiOrder[] => []),
      ]);
      setDbItems(items);
      setDbCategories(cats);
      const tableNameById = new Map<string, string>();
      if (tablesData && tablesData.length > 0) {
        const mappedTables: TablePOS[] = tablesData.map((t: any) => ({
          id: t.id,
          name: t.name,
          capacity: t.capacity,
          status: toTableStatus(t.status),
          runningOrder: t.activeOrderId || undefined
        }));
        mappedTables.forEach((table) => tableNameById.set(table.id, table.name));
        setTablesList(mappedTables);
        if (!mappedTables.some((t) => t.name === selectedTable)) {
          setSelectedTable(mappedTables[0].name);
        }
      }
      setLiveOrders(
        ordersData
          .map((order) => mapApiOrderToLiveOrder(order, tableNameById))
  ...
  const persistedMenuIds = useMemo(() => new Set(dbItems.map((item) => item.id)), [dbItems]);
  const matchedSelectedTable = useMemo(
    () => tablesList.find((table) => table.name === selectedTable),
    [selectedTable, tablesList],
  );
  const hasUnsavedCartItems = billingCart.some(
    (item) => !persistedMenuIds.has(item.menuItemId),
  );
  const checkoutDisabledReason = useMemo(() => {
    if (checkoutBusy) return 'Checkout is already processing.';
    if (loading) return 'Menu and table data is still loading.';
    if (!shiftOpened) return 'Open the cash register before checkout.';
    if (billingCart.length === 0) return 'Cart is empty.';
    if (isTakeaway || isDelivery) {
      return 'POS checkout currently requires a seated table.';
    }
    if (!matchedSelectedTable) return 'Select a valid table before checkout.';
    if (hasUnsavedCartItems) {
      return 'Only saved menu items can be checked out.';
    }
    return null;
  }, [
    billingCart.length,
  ...
  const handleCheckout = async () => {
    if (checkoutDisabledReason) {
      setCheckoutError(checkoutDisabledReason);
      triggerToast(checkoutDisabledReason);
      return;
    }
    ...
    try {
      const payload = {
        ...
        tableId: matchedSelectedTable!.id,
      ...
      const settled = await checkoutOrder(created.id, {
      ...
      const settledOrder = settled.order;

      const newOrder: LiveOrder = {
        id: settledOrder.id,
        orderNumber: settledOrder.orderNumber,
        tableName: isTakeaway ? 'Takeaway' : (isDelivery ? 'Delivery' : selectedTable),
        customerName: settledOrder.customerName || 'Walk-in Guest',
        customerPhone: settledOrder.customerPhone || undefined,
        items: [...billingCart],
        subtotal: Number(settled.invoice.subtotal),
        taxes: Number(settled.invoice.tax),
        serviceCharge: Number(settled.invoice.serviceCharge),
        total: Number(settled.invoice.grandTotal),
        status: 'Paid',
        isTakeaway,
        isDelivery,
        ...
        discountApplied: Number(settled.invoice.discount)
      };
      ...
      // Refresh tables list and active register balance
      void reloadRegisterSession();
      const tablesData = await getTables().catch(() => []);
      if (tablesData && tablesData.length > 0) {
        const mappedTables: TablePOS[] = tablesData.map((t: any) => ({
          id: t.id,
          name: t.name,
      ...
      const message = err?.response?.data?.message || err.message || 'Failed to complete checkout transaction.';
  ...
  const settleExistingOrder = async (order: LiveOrder) => {
    if (order.status === 'Paid') return;
    if (!shiftOpened) {
      const message = 'Open the cash register before billing an order.';
      setCheckoutError(message);
      triggerToast(message);
      return;
    }
    ...
    const serviceCharge = Math.round((taxable * serviceChargePercent) / 100);
    const amount = taxable + tax + serviceCharge;
    ...
    try {
      ...
      setLiveOrders(prev =>
        prev.map((current) => (current.id === order.id ? paidOrder : current)),
      );
      setPrintedOrder(paidOrder);
      setShowReceipt(true);
      triggerToast('Invoice updated to PAID status.');
      void reloadRegisterSession();
    } catch (err: any) {
      const message = err?.response?.data?.message || err.message || 'Failed to settle order.';
      setCheckoutError(message);
      triggerToast(message);
    } finally {
      setCheckoutBusy(false);
  ...
  const handleCashEntry = async (type: 'IN' | 'OUT') => {
    ...
    try {
      await addRegisterCashLog({
        amount,
        type: type === 'IN' ? 'CASH_IN' : 'CASH_OUT',
        reason: cashInOutReason,
      });
      triggerToast(`Cash ${type === 'IN' ? 'In' : 'Out'} logged: ${type === 'IN' ? '+' : '-'}₹${amount}`);
      if (type === 'IN') setCashInVal('');
      else setCashOutVal('');
      setCashInOutReason('');
      void reloadRegisterSession();
    } catch (err) {
      console.error(err);
      triggerToast('Failed to log transaction.');
    }
  };
  ...
  const totalSalesToday = useMemo(() => {
    return liveOrders.filter(o => o.status === 'Paid').reduce((sum, o) => sum + o.total, 0);
  }, [liveOrders]);
    ...
    <div className="space-y-6 text-slate-900 bg-slate-50/50 p-1 print:p-0 print:bg-white min-h-screen max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_420px] no-print">
            ...
            <div className="rounded-xl bg-white border border-slate-200 p-5 shadow-xs space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                ...
                {filteredCatalog.length === 0 && (
                  <div className="sm:col-span-2 md:col-span-3 rounded-xl border border-dashed border-slate-200 bg-slate-50 py-10 text-center text-xs font-bold text-slate-400">
                    No active menu items available for billing.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm space-y-4 flex flex-col justify-between self-start">
            ...
            <div className="border-t border-slate-200 pt-3.5 space-y-4">
              ...
              {checkoutError && (
                <div className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[11px] font-bold text-rose-700">
                  {checkoutError}
                </div>
              )}
              ...
              <button
                ...
                title={checkoutDisabledReason || 'Checkout order'}
              ...
              >
                {checkoutBusy ? 'Processing Checkout...' : 'Checkout & Send KOT (F4)'}
              </button>
            </div>
      ...
      {activeTab === 'live-orders' && (
        <section className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm space-y-6 no-print">
          ...
          {loading ? (
            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-8 text-center text-xs font-bold text-slate-500">
              Loading live orders...
            </div>
          ) : liveOrders.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center text-xs font-bold text-slate-500">
              No active restaurant orders are waiting at POS.
            </div>
          ) : (
            ...
            {liveOrders.map((order) => (
              <div key={order.id} className="bg-slate-50 border border-slate-200/80 rounded-xl p-4 space-y-3 relative overflow-hidden">
                ...
                <div className="flex gap-2 pt-2 border-t border-slate-100">
                  ...
                  {order.status !== 'Paid' && (
                    <button
                      onClick={() => void settleExistingOrder(order)}
                      disabled={checkoutBusy || !shiftOpened}
                      className="flex-1 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-100 disabled:text-slate-400 text-white py-2 rounded-lg text-xs font-bold transition"
                    >
                      {checkoutBusy ? 'Billing...' : 'Bill'}
                    </button>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm space-y-3">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                <h3 className="text-sm font-bold text-slate-800">Register Session</h3>
                <span className={`rounded px-2 py-0.5 text-[10px] font-bold uppercase ${
                  shiftOpened ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'
                }`}>
                  {shiftOpened ? 'Open' : 'Closed'}
                </span>
              </div>

              {!shiftOpened ? (
                <div className="space-y-2">
                  <label className="text-[10px] text-slate-500 font-bold block">Opening Balance</label>
                  <input
                  ...
                  <button
                    ...
                    disabled={registerBusy || Number(openingBalanceInput) < 0}
                    className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-100 disabled:text-slate-400 text-white text-xs font-bold py-2 rounded-lg transition"
                  >
                    {registerBusy ? 'Opening Register...' : 'Open Register'}
                  </button>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3 text-xs font-bold text-slate-700">
                  <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
                    <span className="block text-[10px] uppercase text-slate-400">Opening</span>
                    <span>₹{openingBalance}</span>
                  </div>
                  <div className="rounded-lg bg-slate-50 border border-slate-200 p-3">
                    <span className="block text-[10px] uppercase text-slate-400">Current</span>
                    <span>₹{currentDrawerCash}</span>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-3.5 pt-2">
                <div className="space-y-1">
                  ...
                  <button
                    ...
                    disabled={!shiftOpened || registerBusy}
                    className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-100 disabled:text-slate-400 text-white text-xs font-bold py-1.5 rounded-lg transition"
                  >
                    + Cash In
                  </button>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <button
                    ...
                    disabled={!shiftOpened || registerBusy}
                    className="w-full bg-rose-600 hover:bg-rose-700 disabled:bg-slate-100 disabled:text-slate-400 text-white text-xs font-bold py-1.5 rounded-lg transition"
                  >
                    - Cash Out
                  </button>
                </div>
            ...
            <div className="rounded-xl bg-white border border-slate-200 p-5 shadow-sm text-center space-y-2">
              ...
              <button
                ...
                disabled={!shiftOpened || registerBusy}
                className="w-full bg-slate-900 hover:bg-slate-800 disabled:bg-slate-100 disabled:text-slate-400 text-white text-xs font-bold py-2 rounded-lg transition cursor-pointer"
              >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-white rounded-2xl p-5 border border-slate-200 shadow-2xl space-y-4">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">Close Register Session</h3>
                <p className="text-[10px] text-slate-400 font-semibold mt-0.5">Confirm counted cash before settlement.</p>
              </div>
              <button onClick={() => setShowCloseModal(false)} className="p-1 hover:bg-slate-50 rounded">
                <X size={15} className="text-slate-400 hover:text-slate-700" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="font-bold text-slate-500 block mb-1">Closing Balance</label>
                <input
                  value={closingBalanceInput}
                  onChange={(e) => setClosingBalanceInput(e.target.value)}
                  type="number"
                  min="0"
                  className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg outline-none focus:border-indigo-500 font-bold text-slate-800"
                />
              </div>
              <div>
                <label className="font-bold text-slate-500 block mb-1">Notes</label>
                <textarea
                  value={closingNotesInput}
                  onChange={(e) => setClosingNotesInput(e.target.value)}
                  rows={3}
                  className="w-full bg-slate-50 border border-slate-200 px-3 py-2 rounded-lg outline-none focus:border-indigo-500 text-slate-800 resize-none"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2">
              ...
              <button
                ...
                disabled={registerBusy || closingBalanceInput === ''}
                className="rounded-lg bg-slate-900 py-2 text-xs font-bold text-white hover:bg-slate-800 disabled:bg-slate-100 disabled:text-slate-400"
              >
