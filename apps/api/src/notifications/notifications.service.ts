...
export class NotificationsService {
  ...
  async sendNotification(input: SendNotificationInput): Promise<any> {
    this.logger.log(`Dispatching notification. Trigger: ${input.trigger ?? 'None'}`);

    let title = input.title ?? 'Alert';
    let body = input.body ?? '';
    let type = input.type ?? 'INFO';
    ...
    if (input.userIds && input.userIds.length > 0) {
      ...
      const users = await this.prisma.users.findMany({
        where: {
          ...
          restaurantId: input.tenantId || undefined,
          isActive: true,
        },
        select: { id: true },
      });
      ...
      const users = await this.prisma.users.findMany({
        where: {
          ...
          role: { in: ['RESTAURANT_OWNER', 'MANAGER', 'CASHIER', 'WAITER', 'CHEF'] },
          isActive: true,
        },
        select: { id: true },
      });
    ...
    for (const userId of targetUserIds) {
      ...
      const allowedChannels = userPrefs ? userPrefs.channels : configuredChannels;
    ...
    return { success: true, notificationId: notification.id, recipientCount: targetUserIds.length };
  }
  ...
  async getNotifications(
    user: JwtUser,
    filter?: { read?: boolean; category?: string; limit?: number; page?: number },
  ) {
